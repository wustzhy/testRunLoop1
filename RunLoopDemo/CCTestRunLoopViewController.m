...
- (void)viewDidLoad
{
    ...
    UIButton *normalThreadButton = [[UIButton alloc] initWithFrame:CGRectMake(10, 30+64, 200, 50)];
    ...
    UIButton *runLoopThreadButton = [[UIButton alloc] initWithFrame:CGRectMake(10, 100+64, 200, 50)];
    ...
    /* 测试Button 看UI是否能正常响应 */
    UIButton *normalButton = [[UIButton alloc] initWithFrame:CGRectMake(10, 170+64, 200, 50)];
    ...
    {
        UIButton *testCustomSourceButton = [[UIButton alloc] initWithFrame:CGRectMake(10, 300+64, 300, 50)];
...
-(void)setNav{
    self.title = @"test runLoop";
//    UIButton * pushBtn = [UIButton new];
//    pushBtn.backgroundColor = [UIColor whiteColor];
//    [pushBtn addTarget:self action:@selector(clickToPushToTableView) forControlEvents:UIControlEventTouchUpInside];
//    [pushBtn setTitle:@"to Table" forState:UIControlStateNormal];
//    pushBtn.titleLabel.font = [UIFont systemFontOfSize:13];
//    pushBtn.bounds  = CGRectMake(0, 0, 100, 40);
//    UIBarButtonItem * barBtn = [[UIBarButtonItem alloc]initWithCustomView:pushBtn];
    UIBarButtonItem * barB = [[UIBarButtonItem alloc]initWithTitle:@"to Table" style:UIBarButtonItemStyleBordered target:self action:@selector(clickToPushToTableView)];
    self.navigationItem.rightBarButtonItem = barB;
    
}
-(void)clickToPushToTableView{
    NSLog(@"click");
...
- (void)handleNormalThreadButtonTouchUpInside
{
    ...
    while (!self.normalThreadDidFinishFlag) {
        NSLog(@"wait and judge finished or not ");
...
- (void)handleRunLoopThreadButtonTouchUpInside
{
    ...
    NSLog(@"After ending runLoop,current thread == %@",runLoopThread);   // 5   
}
...
- (void)handleRunLoopThreadTask
{
    ...
    for (NSInteger i = 0; i < 5; i ++) {
        NSLog(@"In Run Loop Thread, count = %ld, then sleep 1", i);
        sleep(1);
    }
...
#else
    [self performSelectorOnMainThread:@selector(updateRunLoopThreadDidFinishFlag) withObject:nil waitUntilDone:NO];     // go to main thread to handle
#endif
